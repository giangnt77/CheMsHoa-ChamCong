...
<b>Nhân viên:</b> ${swapData.requester_name}
<b>Ngày:</b> ${formattedDate}
<b>Ca gốc:</b> ${swapData.original_time || swapData.my_shift_info || 'Không rõ'}
<b>Thực tế:</b> ${swapData.adjusted_time || 'Chưa nhập'}
${extraHoursStr ? `<b>Chênh lệch:</b> <code>${extraHoursStr}</code>\n` : ''}<b>Lý do:</b> "${swapData.reason}"
    `.trim();
  } else {
    textMessage = `
...
<b>Nhân viên:</b> ${swapData.requester_name}
<b>Đổi với:</b> ${swapData.target_employee_name || 'Đồng nghiệp'}
<b>Ngày:</b> ${formattedDate}
<b>Ca của ${swapData.requester_name}:</b> ${swapData.my_shift_info || 'Có ca làm'}
<b>Ca của bạn đổi:</b> ${swapData.target_shift_info || 'Không có ca'}
<b>Lý do:</b> "${swapData.reason}"
    `.trim();
  }
