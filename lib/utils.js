...
export async function sendTelegramNotification(swapData) {
  ...
  let botToken = process.env.NEXT_PUBLIC_TELEGRAM_BOT_TOKEN;
  let chatId = process.env.NEXT_PUBLIC_TELEGRAM_CHAT_ID;
  ...
  const textMessage = `
🔄 *YÊU CẦU ĐỔI CA MỚI — CHÈ MS HOA*

👤 *Người xin đổi:* ${swapData.requester_name}
🤝 *Nhờ làm hộ / Đổi ca với:* ${swapData.target_employee_name}
📅 *Ngày diễn ra đổi:* ${formattedDate}
⏰ *Ca của ${swapData.requester_name}:* ${swapData.my_shift_info}
🔁 *Ca của ${swapData.target_employee_name}:* ${swapData.target_shift_info}
💬 *Lý do xin đổi:* "${swapData.reason}"

👉 *Vui lòng vào trang Admin để phê duyệt yêu cầu này!*
  `.trim();
