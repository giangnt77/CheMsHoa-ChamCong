...
export async function sendTelegramNotification(swapData) {
  ...
  const DEFAULT_BOT_TOKEN = '8514257668:AAFjq2t3a9p--jmwLomShVX4HSOJ8WNyIGw';
  const DEFAULT_CHAT_ID = '5766522088';
  ...
  if (typeof window !== 'undefined') {
    ...
    if (customToken) botToken = customToken;
    if (customChatId) chatId = customChatId;
  }
  ...
  const textMessage = `
<b>🔄 YÊU CẦU ĐỔI CA MỚI — CHÈ MS HOA</b>

<b>👤 Người xin đổi:</b> ${swapData.requester_name}
<b>🤝 Nhờ làm hộ / Đổi ca với:</b> ${swapData.target_employee_name}
<b>📅 Ngày diễn ra đổi:</b> ${formattedDate}
<b>⏰ Ca của ${swapData.requester_name}:</b> ${swapData.my_shift_info}
<b>🔁 Ca của ${swapData.target_employee_name}:</b> ${swapData.target_shift_info}
<b>💬 Lý do xin đổi:</b> "${swapData.reason}"

👉 <i>Vui lòng vào trang Admin để phê duyệt yêu cầu này!</i>
  `.trim();
