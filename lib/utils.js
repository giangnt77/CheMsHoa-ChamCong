...
/**
 * Lấy tháng hiện tại dạng YYYY-MM theo chuẩn giờ Việt Nam (GMT+7)
 * Sử dụng toán học UTC+7 thuần túy, an toàn 100% trên mọi thiết bị và hệ điều hành (kể cả iPhone 7/8, iOS cũ)
 */
...
/**
 * Lấy ngày hiện tại dạng YYYY-MM-DD theo chuẩn giờ Việt Nam (GMT+7)
 * Sử dụng toán học UTC+7 thuần túy, an toàn 100% trên mọi thiết bị và hệ điều hành (kể cả iPhone 7/8, iOS cũ)
 */
...
/**
 * Chuyển ngày YYYY-MM-DD sang chuỗi ISO an toàn giữ nguyên ngày tại Việt Nam (GMT+7)
 */
export function toVNISOString(dateStr) {
  if (!dateStr) return new Date().toISOString();
  const cleanDate = String(dateStr).split('T')[0].trim();
  return `${cleanDate}T12:00:00+07:00`;
}

export function formatDateISO(date) {
  if (!date) return '';
  if (typeof date === 'string') {
    const clean = date.split('T')[0].trim();
...
export function getMondayOfCurrentWeek() {
  ...
  const day = todayDate.getDay(); // 0=CN, 1=T2...
...
/**
 * Gửi thông báo tự động qua Telegram Bot về Yêu Cầu Đổi Ca Mới (Bảo mật qua Server API Route)
 */
export async function sendTelegramNotification(swapData) {
  ...
  if (typeof window !== 'undefined') {
    customToken = localStorage.getItem('chems_telegram_bot_token') || '';
    customChatId = localStorage.getItem('chems_telegram_chat_id') || '';
    if (customToken === '8840577376:AAFLKRa3e8e4wXFcu6hVXBuI6fJdo4WbPR8') {
      customToken = '8903834760:AAEdo7C9zdoj5uC63nMc_rnh7MWMavOhb98';
      localStorage.setItem('chems_telegram_bot_token', customToken);
    }
  }
...
<b>Lý do:</b> "${reasonText}"
  ...
  try {
    ...
    if (customChatId) payload.chatId = customChatId;
