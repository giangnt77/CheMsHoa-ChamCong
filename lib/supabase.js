...
export async function deleteEmployee(id) {
  checkSupabase();
  // Cascade delete all associated records first to ensure no leftover orphaned data in Supabase
  await supabase.from('schedule').delete().eq('employee_id', id);
  await supabase.from('availability').delete().eq('employee_id', id);
  await supabase.from('penalties').delete().eq('employee_id', id);
  await supabase.from('employee_rates').delete().eq('employee_id', id);
