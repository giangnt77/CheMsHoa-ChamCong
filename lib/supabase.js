...
export async function getEmployees() {
  ...
  return (data || []).filter((e) => !e.status || e.status === 'active');
}
...
export async function updateEmployeeStatus(id, status) {
  ...
  try {
    ...
    if (error) {
      ...
      return { ...fbData, status };
    }
    return data;
  } catch (err) {
    console.warn('updateEmployeeStatus fallback:', err);
    return { id, is_active: isActive, status };
  }
}
...
export function calculateSalaryFromShifts(shifts = [], rates = [], defaultRate = 20000) {
  ...
  // Sắp xếp rates theo effective_date tăng dần
